...
while jogando:
    ...
    if navios_afundados == 10:
        ...
        pp_op=[]
        lo=random.randint(linha)
        co=random.randint(coluna)
        if [lo,co] in pp_op:
            print("Seu oponente está atacando na linha {lo} e coluna {co}")
        tabuleiro_jogador = faz_jogada(tabuleiro_jogador, lO,cO)
        navios_afundados_j = afundados(frota, tabuleiro_jogador)
            if navios_afundados_j ==10:
